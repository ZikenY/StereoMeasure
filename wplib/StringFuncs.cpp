...
bool validatedate1y(const std::string& line)
{
    long y = StrToInt(RemoveExtNamePart(RemoveExtNamePart(line)));
    long m = StrToInt(GetExtNamePart(RemoveExtNamePart(line)));
    long d = StrToInt(GetExtNamePart(line));
    ...
    long now_y = StrToInt(std::string(buffdate));
    strftime(buffdate, 100, "%m", ptr);
    long now_m = StrToInt(std::string(buffdate));
    strftime(buffdate, 100, "%d", ptr);
    long now_d = StrToInt(std::string(buffdate));
    ...
    if (((now_y-y) == 1) && (now_m > m))
    {
        r = false;
    }

    if (((now_y-y) == 1) && (now_m == m) && (now_d > d))
    {
        r = false;
    }
